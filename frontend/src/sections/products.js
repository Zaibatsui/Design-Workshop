...
`.trim();
  ...
  const js = iife(
    cls,
    `var track=root.querySelector("[data-ns-track]");var prev=root.querySelector("[data-ns-prev]");var next=root.querySelector("[data-ns-next]");if(track){var ap=root.getAttribute("data-ns-autoplay")==="1";var interval=parseInt(root.getAttribute("data-ns-interval"),10)||4000;var poh=root.getAttribute("data-ns-poh")!=="0";var total=track.querySelectorAll(".ns-card").length;if(total){var timer=null;function amt(){var c=track.querySelector(".ns-card");return c?c.offsetWidth+18:0;}function idx(){var a=amt();return a?Math.round(track.scrollLeft/a):0;}function go(i){var a=amt();if(!a)return;var w=((i%total)+total)%total;track.scrollTo({left:w*a,behavior:"smooth"});}function start(){if(!ap||total<2)return;stop();timer=setInterval(function(){go(idx()+1);},interval);}function stop(){if(timer){clearInterval(timer);timer=null;}}if(prev)prev.addEventListener("click",function(){go(idx()-1);start();});if(next)next.addEventListener("click",function(){go(idx()+1);start();});if(poh){root.addEventListener("mouseenter",stop);root.addEventListener("mouseleave",start);}start();}}${liveJs}`
  );
