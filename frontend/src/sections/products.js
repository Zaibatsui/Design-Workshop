...
`.trim();
  ...
  const js = iife(
    cls,
    `var track=root.querySelector("[data-ns-track]");var prev=root.querySelector("[data-ns-prev]");var next=root.querySelector("[data-ns-next]");if(track){var ap=root.getAttribute("data-ns-autoplay")==="1";var interval=parseInt(root.getAttribute("data-ns-interval"),10)||4000;var poh=root.getAttribute("data-ns-poh")!=="0";var timer=null,snapT=null;var orig=track.querySelectorAll(".ns-card");if(orig.length>1){var fc=orig[0].cloneNode(true);fc.setAttribute("data-ns-clone","1");track.appendChild(fc);var lc=orig[orig.length-1].cloneNode(true);lc.setAttribute("data-ns-clone","1");track.insertBefore(lc,orig[0]);var initAmt=orig[0].offsetWidth+18;track.scrollLeft=initAmt;}function step(dir){var c=track.querySelector(".ns-card");if(!c)return;var amt=c.offsetWidth+18;var max=track.scrollWidth-track.clientWidth;var sl=track.scrollLeft;if(sl>=max-5)track.scrollLeft=amt;else if(sl<=5)track.scrollLeft=max-amt;track.scrollBy({left:dir*amt,behavior:"smooth"});if(snapT)clearTimeout(snapT);snapT=setTimeout(function(){var s2=track.scrollLeft;var m2=track.scrollWidth-track.clientWidth;if(s2>=m2-5)track.scrollLeft=amt;else if(s2<=5)track.scrollLeft=m2-amt;},550);}function start(){if(!ap)return;stop();timer=setInterval(function(){step(1);},interval);}function stop(){if(timer){clearInterval(timer);timer=null;}}if(prev)prev.addEventListener("click",function(){step(-1);start();});if(next)next.addEventListener("click",function(){step(1);start();});if(poh){root.addEventListener("mouseenter",stop);root.addEventListener("mouseleave",start);}start();}${liveJs}`
  );
