 ...
 * manager card) can each be placed in one of nine grid cells. Lets one
 * tool serve very different brand looks without changing the underlying
 * snippet contract.
 *
 * All blocks render transparent (no card chrome) — readability is
 * controlled via the background overlay opacity slider and the heading
 * / body text colour.
 *
...
.${cls} .ns-am-name{font-size:16px;font-weight:600;color:var(--ns-text);line-height:1.3}
.${cls} .ns-am-role{font-size:13px;opacity:.75;color:var(--ns-text);line-height:1.3;margin-bottom:6px}
.${cls} .ns-am-link{display:inline-flex;align-items:center;gap:8px;font-size:13px;color:var(--ns-text);text-decoration:none;line-height:1.6}
